while getopts  ":i:f:t:h" o; do
	...
	t) t=${OPTARG}
            ;;
	h) echo "
			This script is used to align nucleotide MSA based on the relative protein MSA by a combination of transeq & mafft & pal2nal.
...
################################################################################################################
	...
	for j in *.fa*; do export n=$(echo $j | awk -F "." '{print $1}');
		...
		perl ~/pal2nal.pl $n.mafft.p.aln $n.n.fasta -output fasta >> tmp1.aln;
